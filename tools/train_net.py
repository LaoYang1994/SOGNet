...
from sognet.lib.utils.metric import AvgMetric
from sognet.lib.nn.optimizer import SGD, Adam, clip_grad
...
def sognet_train():
    ...
    if config.network.has_relation:
        metrics.extend([AvgMetric(name='relation_loss'), ])
        # metrics.extend([AvgMetric(name='relation_loss'), AvgMetric(name='true_rel_loss'), AvgMetric(name='relation_acc')])
        metrics_name.extend(['relation_loss',])
        # metrics_name.extend(['relation_loss', 'true_rel_loss', 'relation_acc'])
        ...
        if config.train.use_horovod:
            ...
            while inner_iter + num_gpus <= len(train_loader):
                ...
                if config.network.has_panoptic_head:
                    loss = loss + output['panoptic_loss'].mean() * config.train.panoptic_loss_weight
                if config.network.has_relation:
                    loss = loss + output['relation_loss'].mean() * config.train.relation_loss_weight
                loss.backward()
                optimizer.step(lr)
