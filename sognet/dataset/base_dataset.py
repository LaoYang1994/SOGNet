...
from sognet.bbox import bbox_transform
...
class BaseDataset(torch.utils.data.Dataset):
    ...
    def collate(self, batch):
        ...
        for key in batch[0]:
            if key == 'data':
            ...
            elif key == 'matched_mask':
                blob.update({'matched_mask': torch.from_numpy(self.gt_list_to_blob([b['matched_mask'] for b in batch], scale=1./4))})
            elif key == 'im_info':
                ...
                assert len(batch) == 1
                blob.update({'relation': batch[0]['relation']})
